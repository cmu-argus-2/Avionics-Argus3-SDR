...
static const iq16_t parsed_iq_samples[] = {
#include "parsed_iq.txt"
};

#define PARSED_IQ_COUNT ((int)(sizeof(parsed_iq_samples) / sizeof(parsed_iq_samples[0])))

static int g_sample_rd_idx = 0;
...
/* ---------------- sample source from parsed_iq.txt ---------------- */

static int read_gnss_samples_1ms(iq16_t *dst, int samples_per_ms)
{
    if (PARSED_IQ_COUNT <= 0) {
        return 0;
    }

    for (int i = 0; i < samples_per_ms; i++) {
        dst[i] = parsed_iq_samples[g_sample_rd_idx];
        g_sample_rd_idx++;
        if (g_sample_rd_idx >= PARSED_IQ_COUNT) {
            g_sample_rd_idx = 0;   /* wrap around */
        }
    }
...
int main(void)
{
    ...
    {
        uint64_t t = rel_us();
        printf("[T=%lu.%06lu s] [TB] GNSS parsed-IQ testbench start\r\n", TS_SEC(t), TS_USEC(t));
    }
    ...
    if (!read_gnss_samples_1ms(raw_1ms, samples_per_ms)) {
        printf("[ERR] failed to load parsed IQ samples\r\n");
        return -1;
    }
    ...
    for (int iter = 0; iter < TEST_ITERATIONS; iter++) {
        if (!read_gnss_samples_1ms(raw_1ms, samples_per_ms)) {
            printf("[ERR] failed to read parsed IQ block at iter=%d\r\n", iter);
            continue;
        }
